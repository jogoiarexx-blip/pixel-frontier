...
import type { PixelPart } from "./types";

const materialCache = new Map<string, StandardMaterial>();

function colorMaterial(scene: Scene, hex: string) {
...
export function sprite(scene: Scene, name: string, parts: PixelPart[], z = 0) {
...
export function assetSprite(scene: Scene, name: string, assetUrl: string, width: number, height: number, z = 0, yOffset = 0) {
  const root = new TransformNode(name, scene);
  root.position.z = z;
...
export function assetSheetSprite(
  scene: Scene,
  name: string,
  assetUrl: string,
  frameCount: number,
...
export function setAssetFrame(root: TransformNode, frame: number) {
  const data = root.metadata?.spriteSheet as { texture?: Texture; frameCount?: number; currentFrame?: number } | undefined;
  if (!data?.texture || !data.frameCount) return;
  const normalized = ((Math.floor(frame) % data.frameCount) + data.frameCount) % data.frameCount;
  if (data.currentFrame === normalized) return;
...
export interface TextBoard {
  root: TransformNode;
  setText: (title: string, lines: string[], highlight?: number) => void;
...
export function textBoard(
  ...
  const setText = (title: string, lines: string[], highlight = -1) => {
...
export function meter(scene: Scene, name: string, width: number, color: string, parent: TransformNode, x: number, y: number) {
  const root = new TransformNode(name, scene);
  root.parent = parent;
